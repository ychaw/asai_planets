...
def main(neighbors):
    ...
    for i, mass_pred_data in enumerate([pred_data[i:i + len(pred_data) // len(M)] for i in range(0, len(pred_data), len(pred_data) // len(M))]):

        mass_pred_data.resize((resolution_per_axis, resolution_per_axis))
        im = Image.fromarray(np.uint8(np.interp(mass_pred_data, [0, 1], [0, 255])))
...
# Number of neighbors (tests resulted in 9 having the lowest error)
main(9)
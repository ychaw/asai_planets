...
class Simulation():
    ...
    def start(self):
        ...
        while self.running:
            ...
            date_display = font.render(self.date.strftime(
                "%d %b %Y, %H:%M"), False, (200, 200, 200))
            ...
            for entity in self.solar_system.entities:
                ...
                relative_scale = self.scale / self.default_scale
                x = int(relative_scale *
                        ((self.scale * entity.x) + self.dx) + self.offsetx)
                # reflected across y-axis to compensate for pygame's reversed axes
                y = int(relative_scale *
                        ((self.scale * -entity.y) + self.dy) + self.offsety)
